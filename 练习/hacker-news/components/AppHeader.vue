<template>
    <header class="app-header">
        <nav class="center-body header-nav" role="navigation">
            <NuxtLink class="nav-logo" to="/" aria-label="Go to home">
            ...
            <NuxtLink v-for="(list, key) in feedsInfo" :key="key" :to="`/${key}`"
                :class="['nav-item', { 'nav-item-active': $route.path.startsWith(`/${key}`) }]">
                {{ list.title }}
            </NuxtLink>

            <span class="nav-github">
                <a class='nav-item' href="https://github.com/nuxt/hackernews" target="_blank" rel="noopener banner">
                    Open on GitHub
                </a>
            </span>
        </nav>
