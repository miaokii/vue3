...
const defaultOptions: AxiosOptions = {
  baseURL: globalSettings.environment.baseUrl || "",
  timeout: 10 * 1000, // 超时时间5s
  ...
  requestOptions: {
    ...
    apiUrl: globalSettings.environment.baseUrl,
    // 是否加入时间戳
    isJoinTimestamp: true,
    // 是否加入token
    withToken: true,
    isNeedLoading: false,
    isNeedSign: globalSettings.environment.isNeedAes,
  },
  transform: {
    // 相当于之前响应拦截器里面的配置
    transformRequestHook: (
      ...
      const { code, msg, data, subCode } = responseData;
      console.log(
        "responseData",
        responseData,
        code === responseResultEnum.SUCCESS
      );
      const isSuccess = code === responseResultEnum.SUCCESS;
      if (isSuccess) {
        return data;
      }
      // 根据code做不同的处理
      ...
      switch (code) {
        case responseResultEnum.BUS_ERROR:
          responseSubEnum.ENABLE !== subCode &&
            showToast({
              type: "fail",
              message: tipMessage,
            });
    ...
    beforeRequestHook: (
      ...
      const { withToken, isNeedSign = false, isJoinTimestamp } = requestOptions;
      // 是否需要设置loading
